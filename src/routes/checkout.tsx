  ...
  return (
    <CustomerShell>
      <div className="px-4 py-5">
        ...
        {!items.length ? (
          <p className="mt-6 text-muted">{t("cart.empty")}</p>
        ) : (
          <>
            <section className="mt-6">
              <h2 className="text-sm font-medium text-muted">{t("checkout.address")}</h2>
              <p className="mt-1 font-medium">{location.label}</p>
              <p className="text-sm text-muted">{location.line1}</p>
            </section>
            <section className="mt-6">
              <h2 className="text-sm font-medium text-muted">{t("checkout.pay")}</h2>
              <label className="mt-2 flex min-h-14 items-start gap-3 rounded-[var(--radius-lg)] bg-surface p-3">
                <input type="radio" name="pay" checked={method === "COD"} onChange={() => setMethod("COD")} />
                <span>
                  <span className="block font-medium">{t("checkout.cod")}</span>
                  <span className="block text-sm text-muted">{t("checkout.codHint")}</span>
                </span>
              </label>
              <label className="mt-2 flex min-h-14 items-start gap-3 rounded-[var(--radius-lg)] bg-surface p-3">
                <input
                ...
                <span>
                  ...
                  <span className="block text-sm text-muted">{t("checkout.upiSandboxHint")}</span>
                  <span className="mt-1 block text-xs text-warn">{t("checkout.upiUnavailable")}</span>
                </span>
              </label>
            </section>
            ...
            <div className="mt-6 rounded-[var(--radius-xl)] bg-surface p-4">
              {quote.data ? <QuoteLines lines={quote.data.quote.lines} locale={locale} /> : <p>{t("common.loading")}</p>}
            </div>
            {quote.data?.quote.blockers.length ? (
            ...
            >
              {busy
                ? t("checkout.placing")
                : t("checkout.place", { amount: formatPaise(quote.data?.quote.totalPaise ?? 0, { locale }) })}
            </Button>
          </>
        )}
