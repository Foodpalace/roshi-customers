...
function OrderDetailPage() {
  ...
  if (!order) {
    return (
      <CustomerShell>
        <p className="p-6">{t("common.empty")}</p>
      </CustomerShell>
    );
  }
    ...
    <CustomerShell>
      <div className="px-4 py-5">
        ...
        <ol className="mt-6 space-y-3">
          {CUSTOMER_TRACK_STEPS.map((step, i) => (
            <li key={step} className={i <= idx || order.status === step ? "text-fg" : "text-subtle"}>
        ...
        {order.deliveryOtp && !["DELIVERED", "CANCELLED"].includes(order.status) ? (
          <p className="mt-4 rounded-[var(--radius-lg)] bg-surface p-3 font-medium">
            {t("orders.otp", { code: order.deliveryOtp })}
            <span className="mt-1 block text-sm font-normal text-muted">{t("orders.otpHint")}</span>
          </p>
        ) : null}
        <p className="mt-4 text-sm text-muted">{t("orders.noGps")}</p>
          ...
          <ul className="mb-3 space-y-1 text-sm">
            {order.items.map((it) => (
              <li key={it.name} className="flex justify-between gap-3">
                ...
                <span className="tabular-nums">{formatPaise(it.lineTotalPaise, { locale })}</span>
              </li>
            ))}
          </ul>
          <QuoteLines lines={order.lines} locale={locale} />
        </div>
        {order.restaurantSimulated && !["DELIVERED", "CANCELLED", "REJECTED"].includes(order.status) ? (
          <div className="mt-4 rounded-[var(--radius-lg)] border border-border p-3">
            <p className="text-sm text-muted">{t("orders.simulateHint")}</p>
            <Button className="mt-2" variant="outline" disabled={advance.isPending} onClick={() => advance.mutate()}>
              {t("orders.simulate")}
            </Button>
          </div>
        ) : null}
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <Button variant="outline" asChild>
            <Link to="/r/$slug" params={{ slug: order.summary.restaurantSlug }}>
              {t("orders.reorder")}
            </Link>
          </Button>
          <Button variant="ghost" asChild>
            <Link to="/support">{t("orders.help")}</Link>
          </Button>
        </div>
      </div>
    </CustomerShell>
