...
function rateLimit(key: string, n: number, windowMs: number): boolean {
  const now = Date.now();
...
async function isFirstOrder(userId: string): Promise<boolean> {
  ...
  const rows = await sql<{ c: number }>`select count(*)::int as c from orders where user_id = ${userId} and status not in ('FAILED_PAYMENT','CANCELLED')`;
  return (rows[0]?.c ?? 0) === 0;
}
...
export const placeOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    if (!rateLimit(`order:${context.userId}`, 5, 60_000)) {
      throw new Error("Too many order attempts. Wait a minute.");
    }
    ...
    const sql = await getSql();
    const existing = await sql<{ id: string; public_id: string }>`
    ...
    if (built.result.quote.blockers.length) {
      throw new Error(`Order blocked: ${built.result.quote.blockers.join(", ")}`);
    }
    ...
    await sql`
      ...
      values (
        ...
        ${`${data.idempotencyKey}:pay`}, ${method === "UPI_SANDBOX" ? JSON.stringify({ sandbox: true, note: "Not a real UPI transfer" }) : null}
      )
    `;
    ...
    await sql`
      ...
      values (
        ...
        ${JSON.stringify({ reason: "provider_not_connected", event: "ORDER_PLACED", orderId })}
      )
    `;
    await sql`
      insert into audit_logs (id, actor_user_id, actor_role, action, entity, entity_id, metadata)
      values (${newId("aud")}, ${context.userId}, ${"customer"}, ${"order.place"}, ${"order"}, ${orderId}, ${JSON.stringify({ publicId, method })})
    `;
    await sql`
...
async function getOwnedOrder(userId: string, orderId: string) {
  ...
  const rows = await sql<{
    ...
    data_label: string;
  }>`select id, public_id, user_id, restaurant_id, status, payment_method, payment_status, total_paise, placed_at::text as placed_at, notes, delivery_otp, address_snapshot, data_label from orders where id = ${orderId} and user_id = ${userId}`;
  return rows[0] ?? null;
}
...
export const listMyOrders = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const rows = await sql<{
      ...
      limit 50
    `;
    const orders: OrderSummary[] = rows.map((r) => ({
      id: r.id,
      ...
      dataLabel: r.data_label as OrderSummary["dataLabel"],
    }));
    return { orders };
  });
...
export const getMyOrder = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (!order) return { order: null as OrderDetail | null };
    const sql = await getSql();
    const rst = await sql<{ name: string; slug: string; data_label: string }>`
      select name, slug, data_label from restaurants where id = ${order.restaurant_id}
    `;
    const items = await sql<{ name_snapshot: string; quantity: number; line_total_paise: number; instructions: string | null }>`
    ...
    const lines = await sql<{
      ...
      reason: string;
    }>`select code, name, amount_paise, source, funded_by, reason from order_price_lines where order_id = ${order.id} order by sort_order`;
    const events = await sql<{ to_status: string; created_at: string; note: string | null }>`
    ...
    const detail: OrderDetail = {
      summary: {
        ...
        dataLabel: order.data_label as OrderSummary["dataLabel"],
      },
      status: order.status,
      ...
      lines: lines.map((l) => ({
        ...
        fundedBy: (l.funded_by ?? "CUSTOMER") as OrderDetail["lines"][number]["fundedBy"],
        reason: l.reason,
      })),
      items: items.map((i) => ({
...
export const advanceSimulatedOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (!order) throw new Error("Order not found");
    const sql = await getSql();
    const rst = await sql<{ data_label: string }>`select data_label from restaurants where id = ${order.restaurant_id}`;
    if (rst[0]?.data_label !== "SIMULATED") {
      throw new Error("Only sample kitchens can be advanced in development.");
    }
